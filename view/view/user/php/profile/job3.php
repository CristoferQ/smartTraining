...
$query = "select count(CAST(job.createdJob  as DATE)) as cantidad, CAST(job.createdJob  as DATE) as fecha from job where job.user = $idUser AND job.tipo_job = 'CLUSTERING' group by CAST(job.createdJob   as DATE)";
