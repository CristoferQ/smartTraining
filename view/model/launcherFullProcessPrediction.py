...
header = ["Algorithm", "Params", "R_Score"]
matrixResponse = []
...
for loss in ['linear', 'squar', 'exponential']:
    for n_estimators in range (10,1501,10):
        try:
            ...
            row = ["AdaBoostClassifier", params, AdaBoostObject.r_score]
...
for bootstrap in [True, False]:
    for n_estimators in range (10,15,10):
        try:
            ...
            row = ["Bagging", params, bagginObject.r_score]
            matrixResponse.append(row)
            iteracionesCorrectas+=1
        except:
            iteracionesIncorrectas+=1
...
for criterion in ['mse', 'friedman_mse', 'mae']:
    for splitter in ['best', 'random']:
        try:
            ...
            row = ["DecisionTree", params, decisionTreeObject.r_score]
            matrixResponse.append(row)
            iteracionesCorrectas+=1
        except:
            iteracionesIncorrectas+=1
            ...
            for min_samples_split in range (2, 11):
                for min_samples_leaf in range(1, 11):
                    try:
                        ...
                        row = ["GradientBoostingClassifier", params, gradientObject.r_score]
                        matrixResponse.append(row)
                        iteracionesCorrectas+=1
                    except:
                        iteracionesIncorrectas+=1
        ...
        for metric in ['minkowski', 'euclidean']:
            for weights in ['uniform', 'distance']:
                try:
                    ...
                    row = ["KNeighborsClassifier", params, knnObect.r_score]
                    matrixResponse.append(row)
                    iteracionesCorrectas+=1
                except:
                    iteracionesIncorrectas+=1
                            ...
                            for max_iter in range(100, 500, 10):
                                for shuffle in [True, False]:
                                    try:
                                        ...
                                        row = ["MLPRegressor", params, MLPObject.r_score]
                                        matrixResponse.append(row)
                                        iteracionesCorrectas+=1
                                    except:
                                        iteracionesIncorrectas+=1
        ...
        for degree in range(3, 15):
            for gamma in [0.01, 0.1, 1, 10, 100]:
                try:
                    ...
                    row = ["NuSVM", params, nuSVM.r_score]
                    matrixResponse.append(row)
                    iteracionesCorrectas+=1
                except:
                    iteracionesIncorrectas+=1
            ...
            for min_samples_leaf in range(1, 11):
                for bootstrap in [True, False]:
                    try:
                        ...
                        row = ["RandomForestRegressor", params, rf.r_score]
                        matrixResponse.append(row)
                        iteracionesCorrectas+=1
                    except:
                        iteracionesIncorrectas+=1
